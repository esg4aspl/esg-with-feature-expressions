...
MAIN="tr.edu.iyte.esgfx.cases.${CASE_NAME}.AutomaticProductConfigurationGenerator_${SHORT_NAME}"
JAVA_OPTS="-Xms$XMS -Xmx$XMX -XX:+UseG1GC"

echo "=== 🚀 STARTING: $N SHARDS ($CASE_NAME) ==="

# Clean old logs
rm -f "${LOG_DIR}/run_proconfiggen_s*.log"

# Start the loop
for i in $(seq 0 $((N-1))); do
  LOG="${LOG_DIR}/run_proconfiggen_s$(printf "%02d" $i).log"
  N_SHARDS=$N SHARD=$i \
...
echo "To monitor logs: tail -f ${LOG_DIR}/run_proconfiggen_s00.log"