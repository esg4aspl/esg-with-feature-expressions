#!/bin/bash
# USAGE: ./RQ1_ComparativeEfficiency_ESGFx_L234.sh <CaseName> <ShortName> <RunID> [TotalShards] [StartShard] [EndShard]
# EXAMPLE: ./RQ1_ComparativeEfficiency_ESGFx_L234.sh SodaVendingMachine SVM 5 40 0 39

# === ARGUMENT VALIDATION ===
if [ -z "$1" ] || [ -z "$2" ] || [ -z "$3" ]; then
  echo "ERROR: Missing required arguments."
  echo "Usage: ./RQ1_ComparativeEfficiency_ESGFx_L234.sh <CaseName> <ShortName> <RunID> [TotalShards] [StartShard] [EndShard]"
  echo "Example: ./RQ1_ComparativeEfficiency_ESGFx_L234.sh SodaVendingMachine SVM 1 4 0 3"
  exit 1
fi
...
for L_VAL in 2 3 4; do
    ...
    for i in $(seq $S_NODE $E_NODE); do
      ...
      LOG="${LOG_DIR}/run_RQ1_ESGFx_L${L_VAL}_runID${RUN_PARAM}_shard$(printf "%02d" $i).log"
