#!/bin/bash
# USAGE: ./RQ3_FaultDetection_EdgeOmitter.sh <CaseName> <ShortName> <RunID> [TotalShards] [StartShard] [EndShard]

if [ -z "$1" ] || [ -z "$2" ] || [ -z "$3" ]; then 
    echo "Error: Missing arguments. Usage: ./RQ3_FaultDetection_EdgeOmitter.sh <Case> <Short> <RunID> [ShardCount] [StartNode] [EndNode]"
    exit 1
fi
...
# FIX: Added missing snapshot jar to classpath (was causing ClassNotFoundException at runtime)
export CP="target/classes:target/dependency/*:target/esg-with-feature-expressions-0.0.1-SNAPSHOT.jar"

MAIN="tr.edu.iyte.esgfx.cases.${CASE_NAME}.RQ3_FaultDetection_EdgeOmitter_${SHORT_NAME}"
JAVA_OPTS="-Xms$XMS -Xmx$XMX -XX:+UseG1GC"

echo "=== EDGE OMISSION EXECUTION STARTED: SHARDS $S_NODE to $E_NODE ==="

for i in $(seq $S_NODE $E_NODE); do
  LOG="${LOG_DIR}/run_RQ3_EdgeOmitter_runID${RUN_PARAM}_shard$(printf "%02d" $i).log"
...
echo "EDGE OMISSION DISPATCHED FOR $CASE_NAME"