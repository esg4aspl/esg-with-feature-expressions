...
public class TotalTimeMeasurement_L1 extends CaseStudyUtilities {
	...
	public void measureToTalTimeForEdgeCoverage() throws Exception {
		...
		System.out.println("Total Time Measurement L=1 " + SPLName + " STARTED");
...
//				System.out.println("Processed " + handledProducts + " products. Current product ID: " + productID);
		...
		if (N_SHARDS > 1) {
			...
			TestSequenceGenerationTimeMeasurementWriter.writeDetailedTimeMeasurement(timeElapsedTotalMs, satTimeMs,
					prodGenTimeMs, testGenTimeMs, handledProducts, shardResultFilePath, SPLName, coverageType);
		} else {
			TestSequenceGenerationTimeMeasurementWriter.writeDetailedTimeMeasurement(timeElapsedTotalMs, satTimeMs,
					prodGenTimeMs, testGenTimeMs, handledProducts,
